...
def main():
    ...
    parser = argparse.ArgumentParser(description="Clean Serato database corruption and duplicates")
    parser.add_argument("--db", type=Path, required=True, help="Path to Database V2 file")
    parser.add_argument("--backup", action="store_true", help="Create backup before cleaning")
    parser.add_argument("--dry-run", action="store_true", help="Analyze only, don't modify database")
    parser.add_argument("--keep-no-metadata", action="store_true", help="Keep tracks without metadata")
    parser.add_argument("--keep-duplicates", action="store_true", help="Keep duplicate tracks")
    ...
    records = read_database_v2_records(args.db)
    ...
    print(f"Records without file path: {analysis['records_without_path']}")
    print(f"Records without metadata: {analysis['records_without_metadata']}")
    print(f"Potential duplicates: {analysis['potential_duplicates']}")
    print(f"Corrupted file paths: {analysis['corrupted_paths']}")
    print(f"File extensions: {analysis['path_statistics']}")
    ...
    cleaned_records, stats = clean_database_records(
        ...
        require_metadata=not args.keep_no_metadata
    )
    
    print(f"Cleanup results:")
    print(f"  Original records: {stats['original_count']}")
    print(f"  Removed (no path): {stats['removed_no_path']}")
    print(f"  Removed (no metadata): {stats['removed_no_metadata']}")
    ...
    print(f"Writing cleaned database...")
    write_database_v2_records(args.db, cleaned_records)
